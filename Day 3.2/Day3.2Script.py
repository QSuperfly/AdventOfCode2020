...
for line in open(input_filename, "r").read().splitlines():
    row = []
    for x in range(len(open(input_filename, "r").read().splitlines())): # Create a lot of columns, just in case. Ugly solution.
...
# Print answer
print(trees_count) # 78 * 178 * 75 * 86 * 39